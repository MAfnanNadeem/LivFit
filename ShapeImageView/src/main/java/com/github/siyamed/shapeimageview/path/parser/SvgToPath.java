...
import com.github.siyamed.shapeimageview.path.xml_parser.MyParser;
...
public class SvgToPath {
    ...
    private SvgToPath(SimpleParser atts) {
        this.atts = atts;
    }
    ...
    void processSvg() throws XmlPullParserException, IOException {
        ...
        do {
            switch (eventType) {
                case SimpleParser.START_DOCUMENT:
    ...
    private void pushTransform(SimpleParser atts) {
    ...
    private String showAttributes(SimpleParser a) {
        String result = "";
    ...
    final Float getFloatAttr(String name, SimpleParser attributes) {
        return getFloatAttr(name, attributes, null);
    }

    final Float getFloatAttr(String name, SimpleParser attributes, Float defaultValue) {
