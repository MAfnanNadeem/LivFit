...
package life.mibo.android.libs.hr
...
import life.mibo.android.libs.hr.apache.queue.CircularFifoQueue
import life.mibo.android.libs.hr.camera.CameraModule
import life.mibo.android.libs.hr.camera.CameraSupport
import java.lang.ref.WeakReference
import java.util.concurrent.atomic.AtomicBoolean


/**
 * Created by <DialogListener href="https://about.me/janrabe">Jan Rabe</DialogListener>.
 */
open class HeartRateMonitor {
    ...
    private var detectionListener: ((Boolean) -> Unit)? = null
    ...
    fun withAverageAfterSeconds(averageTimer: Int): HeartRateMonitor {
    ...
    private fun getScreenDimensions(): Dimension {
        ...
        val display: android.view.Display =
            (context?.get()?.getSystemService(Context.WINDOW_SERVICE) as WindowManager).defaultDisplay
        ...
        if (Build.VERSION.SDK_INT in 14..16) {
            try {
                screenWidth =
                    android.view.Display::class.java.getMethod("getRawWidth").invoke(display) as Int
                screenHeight =
                    android.view.Display::class.java.getMethod("getRawHeight").invoke(display) as Int
    ...
    private var fingerDetected: Boolean = false
        set(value) {
            if (field != value)
                detectionListener?.invoke(value)
            field = value
        }
     ...
     *
     *
     * This method returns `null` for DialogListener `null` input array.
     *
     * @param array  DialogListener `Long` array, may be `null`
     * @return DialogListener `long` array, `null` if null array input
     ...
     * This method returns `null` for DialogListener `null` input array.
     *
     * @param array  DialogListener `Double` array, may be `null`
     * @return DialogListener `double` array, `null` if null array input
     * @throws NullPointerException if array content is `null`
     */
    ...
    private fun log(message: String?) {
        Logger.e("HeartRate $message")
    }

    fun setDetectionListener(fingerDetectionListener: ((Boolean) -> Unit)?): HeartRateMonitor {
        this.detectionListener = fingerDetectionListener
        return this
    }
}