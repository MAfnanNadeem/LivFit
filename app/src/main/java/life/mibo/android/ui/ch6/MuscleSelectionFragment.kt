...
import life.mibo.android.ui.ch6.adapter.ChannelSelectAdapter
...
//        list.add(ChannelSelectAdapter.Item(2, R.drawable.ic_channel_abdomen))
    ...
    private fun getMusclesFromBundle() {
        val suit = stateBundle.getSerializable("program_suit")
        if (suit is Suit) {
            val muscles = suit.muscles
            val list = ArrayList<Muscle>()
            muscles?.forEach { it ->
                it?.let { m ->
                    list.add(m)
    ...
    fun parseMuscles(list: ArrayList<Muscle>) {
