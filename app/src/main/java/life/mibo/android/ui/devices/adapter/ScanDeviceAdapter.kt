package life.mibo.android.ui.devices.adapter
...
//                }
            ...
            if (item.type == DeviceTypes.RXL_WIFI || item.type == DeviceTypes.RXL_BLE) {
                ...
                Logger.e(
                    "PorterDuff.Mode.SRC_OUT $adapterPosition " + life.mibo.android.utils.Utils.getColorAt(
                        adapterPosition
                    )
                )
            ...
            if (item.statusConnected == 1) {
                ...
                if (item.type == DeviceTypes.RXL_WIFI || item.type == DeviceTypes.RXL_BLE) {
                    image?.setImageDrawable(
                        life.mibo.android.utils.Utils.getColorFilterDrawable(
                            image?.context,
                            R.drawable.ic_rxl_pods_icon_200,
                            life.mibo.android.utils.Utils.getColorAt(item.id)
                        )
                    )
