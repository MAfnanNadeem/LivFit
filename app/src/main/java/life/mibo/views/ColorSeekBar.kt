/*
 *  Created by Sumeet Kumar on 1/12/20 8:16 AM
 *  Copyright (c) 2020 . MI.BO All rights reserved.
 *  Last modified 12/22/19 8:09 AM
 *  Mibo Hexa - app
 */
...
class ColorSeekBar(context: Context, attributeSet: AttributeSet) : View(context, attributeSet) {
    ...
    override fun onTouchEvent(event: MotionEvent?): Boolean {
        when (event?.action) {
            ...
            MotionEvent.ACTION_UP -> {
                ...
                invalidate()
                colorChangeListener?.onColorSelectListener(getColor())
            }
        }
        return true
    }
    ...
    interface OnColorChangeListener {
        ...
        fun onColorSelectListener(color: Int)
    }
}