/*
 *  Created by Sumeet Kumar on 2/6/20 11:56 AM
 *  Copyright (c) 2020 . MI.BO All rights reserved.
 *  Last modified 2/6/20 11:56 AM
 *  Mibo Hexa - app
 */
...
import life.mibo.views.FluidSlider.Size.NORMAL
import life.mibo.views.FluidSlider.Size.SMALL
import kotlin.math.*


class FluidSlider constructor(
    context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0, size: Size = Size.NORMAL
    ...
    init {
        ...
        if (attrs != null) {
            ...
            try {
                colorBar = a.getColor(R.styleable.FluidSlider_slider_bar_color, COLOR_BAR)
                colorBubble = a.getColor(R.styleable.FluidSlider_slider_bubble_color, COLOR_LABEL)
                colorBarText = a.getColor(R.styleable.FluidSlider_slider_bar_text_color, COLOR_BAR_TEXT)
                colorBubbleText =
                    a.getColor(R.styleable.FluidSlider_slider_bubble_text_color, COLOR_LABEL_TEXT)

                position = max(
                    0f,
                    min(1f, a.getFloat(R.styleable.FluidSlider_slider_initial_position, INITIAL_POSITION))
                )
                textSize = a.getDimension(R.styleable.FluidSlider_slider_text_size, TEXT_SIZE * density)
                duration =
                    abs(a.getInteger(R.styleable.FluidSlider_slider_duration, ANIMATION_DURATION)).toLong()

                a.getString(R.styleable.FluidSlider_slider_start_text)?.also { startText = it }
                a.getString(R.styleable.FluidSlider_slider_end_text)?.also { endText = it }

                val defaultBarHeight = if (a.getInteger(
                        R.styleable.FluidSlider_slider_size,
                        1
                    ) == 1
                ) NORMAL.value else SMALL.value
