...
/**
 * Created by Sumeet Kumar on 05-07-2018.
 * // for FinPocket
 */
...
public class OtpEditText extends AppCompatEditText implements TextWatcher {
    ...
    public static final String XML_ANDROID = "http://schemas.android.com/apk/res/android";
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        paint.setStrokeWidth(mLineStroke);
    ...
    private void getAttrsFromTypedArray(AttributeSet attributeSet) {
        ...
        mMaxLength = attributeSet.getAttributeIntValue(XML_ANDROID, "maxLength", 6);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < mNumChars; i++) {
            ...
            switch (mBoxStyle) {
                case ROUNDED_UNDERLINE:
                    try {
                        canvas.drawRoundRect(startX, bottom * .95f, startX + mCharSize, bottom, 16, 16, strokePaint);
                    } catch (NoSuchMethodError err) {
                        canvas.drawRect(startX, bottom * .95f, startX + mCharSize, bottom, strokePaint);
                    }
                    break;
                case ROUNDED_BOX:
                    try {
                        ...
                        canvas.drawRect(startX, top, startX + mCharSize, bottom, strokePaint);
                    }
                    break;

                case UNDERLINE:
                    canvas.drawRect(startX, ((float) bottom * .95f), startX + mCharSize, bottom, strokePaint);
                    break;
    ...
    private void updateColorForLines(boolean next, boolean current) {
        if (next) {
            strokePaint.setColor(mSecondaryColor);
            paint.setColor(mSecondaryColor);
        } else {
            strokePaint.setColor(mSecondaryColor);
        ...
        if (current) {
            paint.setColor(ContextCompat.getColor(getContext(), android.R.color.white));
            strokePaint.setColor(mPrimaryColor);
        }
    }
