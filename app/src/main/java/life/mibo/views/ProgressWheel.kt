/*
 *  Created by Sumeet Kumar on 7/2/20 5:02 PM
 *  Copyright (c) 2020 . MI.BO All rights reserved.
 *  Last modified 7/2/20 5:02 PM
 *  Mibo Hexa - app
 */
...
class ProgressWheel : View {
    ...
    private fun setupBounds() {
        ...
        mCountTextWidth =
            mCountTextPaint.measureText(if (mCountText == null || mCountText!!.isEmpty()) " " else mCountText)
...
//        mMarginBtwTexts = a.getDimension(R.styleable.ProgressWheel_marginBtwText, mMarginBtwTexts)
        ...
        // Set up a default TextPaint
        mProgressColor = ContextCompat.getColor(context, R.color.colorPrimary)
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        try {
            ...
            canvas.drawArc(
                mProgressBounds,
                -90f,
                mPercentage.times(3.5).toFloat(),
                false,
                mBarPaint
            )

//            if (mCountText != null && mCountText!!.isNotEmpty()) {
...
//
//
//            if (mDefText != null && mDefText!!.isNotEmpty()) {
...
//            }
    ...
    fun setPercentage(per: Int) {
        ...
        if (per > 50)
            dur = 800L
        startAnimation(per, dur)
    }

    private fun startAnimation(per: Int, duration: Long) {
        //ValueAnimator()
