...
public class IconSwitch extends ViewGroup {
    ...
    private static final int DEFAULT_IMAGE_SIZE_DP = 24;
    ...
    public void setOnClickListener(@Nullable OnClickListener l) {
        ...
        try {
            ...
            for (int i = 0; i < vg.getChildCount(); i++) {
                vg.getChildAt(i).setOnClickListener(l);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void setClickable(boolean clickable) {
        ...
        try {
            ViewGroup vg = (this);
            for (int i = 0; i < vg.getChildCount(); i++) {
                vg.getChildAt(i).setClickable(clickable);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private void init(AttributeSet attr) {
        ...
        int colorDefBackground = ContextCompat.getColor(getContext(), R.color.default_icon);
    ...
    private void onUp(MotionEvent e) {
        Logger.e("notifyCheckedChanged onUp " + lastClickTime + " -- " + SystemClock.elapsedRealtime());
        if (SystemClock.elapsedRealtime() < lastClickTime) {
            Logger.e("notifyCheckedChanged onUp less than 1000 " + lastClickTime);
            clearTouchInfo();
            return;
        }
        ...
        Logger.e("notifyCheckedChanged onUp notifyCheckedChanged");
    }
    ...
    public void setChecked(boolean right) {
        Checked checked = Checked.LEFT;
        if (right)
    ...
    public void toggle() {
        ...
        else currentChecked = Checked.LEFT;
        toggleSwitch();
        notifyCheckedChanged();
    }
    ...
    private void applyPositionalTransform() {
        ...
        int leftColor = Utils.ofArgb(clampedPosition, activeTintIconLeft, inactiveTintIconLeft);
        leftIcon.setColorFilter(leftColor);
        int rightColor = Utils.ofArgb(clampedPosition, inactiveTintIconRight, activeTintIconRight);
        rightIcon.setColorFilter(rightColor);
        int thumbColor = Utils.ofArgb(clampedPosition, thumbColorLeft, thumbColorRight);
