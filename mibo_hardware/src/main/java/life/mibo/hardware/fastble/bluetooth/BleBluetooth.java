...
import life.mibo.hardware.fastble.data.BleConnectStateParameter;
import life.mibo.hardware.fastble.data.BleDevice;
import life.mibo.hardware.fastble.data.BleMsg;
...
public class BleBluetooth {
    ...
    public BleConnector create(String uid) {
        return new BleConnector(this, uid);
    }
    ...
    public synchronized BluetoothGatt connect(BleDevice bleDevice,
       ...
       BleManager.log("connect device: " + bleDevice.getName()
    ...
    private synchronized void refreshDeviceCache() {
        try {
            ...
            if (refresh != null && bluetoothGatt != null) {
                ...
                BleManager.log("refreshDeviceCache, is success:  " + success);
            }
        } catch (Exception e) {
            BleManager.log("exception occur while refreshing device: " + e.getMessage());
            e.printStackTrace();
        }
    }
            ...
            switch (msg.what) {
                case BleMsg.MSG_CONNECT_FAIL: {
                    ...
                    if (connectRetryCount < BleManager.getInstance().getReConnectCount()) {
                        BleManager.log("Connect fail, try reconnect " + BleManager.getInstance().getReConnectInterval() + " millisecond later");
                        ++connectRetryCount;
    ...
    private BluetoothGattCallback coreGattCallback = new BluetoothGattCallback() {
        ...
        public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
            ...
            BleManager.log("BluetoothGattCallback：onConnectionStateChange "
                    + '\n' + "status: " + status
                    + '\n' + "newState: " + newState
                    + '\n' + "currentThread: " + Thread.currentThread().getId());
        ...
        public void onServicesDiscovered(BluetoothGatt gatt, int status) {
            ...
            BleManager.log("BluetoothGattCallback：onServicesDiscovered "
                    + '\n' + "status: " + status
                    + '\n' + "currentThread: " + Thread.currentThread().getId());
