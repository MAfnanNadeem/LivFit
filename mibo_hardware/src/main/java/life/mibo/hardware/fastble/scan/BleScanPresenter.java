...
import life.mibo.hardware.fastble.utils.HexUtil;
...
public abstract class BleScanPresenter implements BluetoothAdapter.LeScanCallback {
    ...
    private void correctDeviceAndNextStep(final BleDevice bleDevice) {
        if (mNeedConnect) {
            BleManager.log("devices detected  ------"
            ...
            mMainHandler.post(new Runnable() {
                @Override
                public void run() {
                    life.mibo.hardware.fastble.scan.BleScanner.getInstance().stopLeScan();
                }
            });
            ...
            if (!hasFound.get()) {
                BleManager.log("device detected  ------"
                        + "  name: " + bleDevice.getName()
                        + "  mac: " + bleDevice.getMac()
                        + "  Rssi: " + bleDevice.getRssi()
