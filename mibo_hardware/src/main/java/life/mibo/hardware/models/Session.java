...
public class Session  implements BaseModel {
    ...
    private TrainerCalendar calendar;


    private ArrayList<User> currentSessionParticipants;
    ...
    public TrainerCalendar getCalendar() {
        return calendar;
    }

    public void setCalendar(TrainerCalendar calendar) {
        this.calendar = calendar;
    }

    public ArrayList<User> getCurrentSessionParticipants() {
        return currentSessionParticipants;
    }


    public void setCurrentSessionParticipants(ArrayList<User> currentSessionParticipants) {
        this.currentSessionParticipants = currentSessionParticipants;
    }
    ...
    public void setUserSelected(String Id) {
        for (User u : currentSessionParticipants) {
    ...
    public User getUserByHrUid(String Uid) {
        for (User u : currentSessionParticipants) {
            if (u.getUserHrMonitor().getUid().equals(Uid)) {
                return u;
            }
        }
        return new User("x", "x");
    ...
    public boolean isColorSelected(int color) {
        for (User u : currentSessionParticipants) {
            if (u.getUserFriendlycolor() == color) {
                return true;
            }
        }
        return false;
    ...
    public void removeCurrentSessionParticipants() {
        currentSessionParticipants.clear();
    }
    ...
    public Program getCurrentSessionProgram() {
        if (currentSessionProgram == null) {
            return getTrainer().getTrainerPrograms().get(0);
        } else {
    ...
    public void checkDeviceStatus(boolean[] status, String uid) {
        Logger.e("Session.java checkStatus: wifi " + status[0] + " ble " + status[1] +
